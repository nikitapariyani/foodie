...
?>
	...
	<?php if ( have_comments() ) : ?>
		<header class="page-header">
			<h2 class="comments-title">
				<?php
					printf( _nx( 'One thought on &ldquo;%2$s&rdquo;', '%1$s thoughts on &ldquo;%2$s&rdquo;', get_comments_number(), 'comments title', 'foodie' ),
		...
		<nav id="comment-nav-above" class="comment-navigation" role="navigation">
			<h5 class="screen-reader-text"><?php _e( 'Comment navigation', 'foodie' ); ?></h5>
			<ul class="pager">
				<li class="nav-previous"><?php previous_comments_link( __( '&larr; Older Comments', 'foodie' ) ); ?></li>
				<li class="nav-next"><?php next_comments_link( __( 'Newer Comments &rarr;', 'foodie' ) ); ?></li>
			</ul>
		...
		<ol class="comment-list media-list">
			<?php
				 ...
				 * to use foodie_comment() to format the comments.
				 * If you want to overload this in a child theme then you can
				 * define foodie_comment() and that will be used instead.
				 * See foodie_comment() in includes/template-tags.php for more.
				 */
				wp_list_comments( array( 'callback' => 'foodie_comment', 'avatar_size' => 50 ) );
			?>
		</ol><!-- .comment-list -->

		<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through ?>
		<nav id="comment-nav-below" class="comment-navigation" role="navigation">
			<h1 class="screen-reader-text"><?php _e( 'Comment navigation', 'foodie' ); ?></h1>
	...
	?>
		<p class="no-comments"><?php _e( 'Comments are closed.', 'foodie' ); ?></p>
	<?php endif; ?>
