...
1. Search for `'foodie'` (inside single quotations) to capture the text domain.
2. Search for `foodie_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `foodie-` to capture prefixed handles.

OR

* Search for: `'foodie'` and replace with: `'megatherium'`
* Search for: `foodie_` and replace with: `megatherium_`
